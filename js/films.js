...
const personalMovieDB ={
    ...
    rememberMyFilms: function(){
        for( let i = 0; i < 2; i++){
            ...
            let nameMovie = prompt("Один из последних просмотренных фильмов", "").trim();
            let ratingMovie = prompt("На сколько оцените его", "").trim();
    ...
    writeYourGenres: function(){
        let genresArr = personalMovieDB.genres;
        for( let i = 1; i < 4; i++){
            let genresName = prompt(`Ваш любимый жанр под номером ${i}`, '');
            if(genresName){
                genresArr.push(genresName) ;
            }
            else{
                i--;
            }
        }
        genresArr.forEach((el, i) => {
